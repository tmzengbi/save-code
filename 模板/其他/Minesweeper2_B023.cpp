...
const char Vers[32] = "Beta 2.1.2.023";
const char Author[32] = "GeForce GTX Titan X";
...
namespace Display{
	...
	bool SyncEnabled = true;// If set false, disable "Sync" algorithm [Experimental]
...
namespace Game{
	...
	Higher 4 bits (0xF0):  128 for 'isLocked' (In certain cases, useful. And overwrite 'isUncovered' as well)
							...
							16 for 'isMine'
	Lower 4 bits (0x0F): Number of mines around (Ranges from 0 to 8)
	*/
	...
	void MainMenuScreen(){
		...
		if(diff==3){
			...
			if(mineCount>(boardWidth*boardHeight*25)/27)mineCount = (boardWidth*boardHeight*25)/27;
		}
	}
	int initGame(){
			...
			while(gameStarted){
				...
				for(i=0;i<boardSize;i++){
					...
					if(cc==0){
						...
						bitByUp(Board(Pos),7);// Bit 'isLocked' here is used as 'isChecked'
						break;
					}
				}
				if(i==boardSize){
					for(i=0;i<boardSize;i++){
						Pos.X = i%boardWidth;
						Pos.Y = i/boardWidth;
						if(isUncovered(Pos))trigger(Pos,0);
					}
				}
				// Win Check
				winchk = true;
